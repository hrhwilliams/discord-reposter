...
async def startposting(j, channel):
    ...
    for message in j:
        ...
        if current_timestamp is None or not (timestamp.day == current_timestamp.day and timestamp.month == current_timestamp.month and timestamp.year == current_timestamp.year):
            message_to_send += f'# {timestamp.year}-{timestamp.month}-{timestamp.day}\n'
            current_timestamp = timestamp

        message_to_send += f'> **{message["author"]["username"]}**: {message["content"]}\n\n'
